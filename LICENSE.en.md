_This is the official English translation. The binding version is the original in Esperanto (file `LICENSE.md`)._

# DamjesaP – Damjes' License v. 2.0

1. Every user has four fundamental freedoms:

    0. The freedom to use the work for any purpose.

    1. The freedom to study how the work functions and to modify it according to one’s own needs.
       Access to the work’s source form is a precondition for this freedom.

    2. The freedom to distribute copies so that the user can, for example, help their neighbor.

    3. The freedom to improve the work and to share one’s improvements with everyone.
       Access to the work’s source form is a precondition for this freedom.

2. No one may revoke or restrict any of the above freedoms for anyone.

3. No one may allow these freedoms to be revoked or restricted
   for anyone in derivative works.

4. Derivative works must clearly state whether they are related or unrelated to the original author,
   including, among other things, a mention of the original author’s name and any possible change of name or branding.

5. The work is provided _as is_, without any express or implied warranty.
   The user uses the work at their own risk.

---

Copyleft 2025 rev. Damjes

This text itself is free and may be distributed under the same terms.
